    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            ...
            embed = discord.Embed(title="Error",
                                  description="""I do not have enough permissions to execute this command. I require the following permissions:\nRead Messages, Send Messages, Embed Links, Add Reactions""",
                                  color=0xff0000)
            ...
            embed = discord.Embed(title="Error",
                                  description="""This command is either restricted, or is temporarily disabled for testing purposes.""",
                                  color=0xff0000)
            await ctx.send(embed=embed)
            return
        elif isinstance(error, commands.CommandOnCooldown):
            embed = discord.Embed(title="Cooldown",
                                  description="""That command is on cooldown, you may use it again once all data has been retrieved.""",
                                  color=0xff0000)
            await ctx.send(embed=embed)
            return
        else:
